...
test.describe("User API Tests", () => {
  ...
  test("Should return 404 for non-existent user", async () => {
    ...
    test('API: Should get user details and attach full response', async () => { 
        const userId = 2; 
        const response = await userApi.getUser(userId); 
        expect(response.ok()).toBeTruthy(); 
        const responseBody = await response.json(); 
 
        // Attach the complete API response payload 
        test.info().attach('API Response Payload', { 
            body: JSON.stringify(responseBody, null, 2), 
            contentType: 'application/json' 
        }); 
        expect(responseBody.data.id).toBe(userId); 
        logger.info(`API request/response attached for user ID: ${userId}.`); 
    });
